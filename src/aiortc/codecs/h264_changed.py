...
MAX_BITRATE = 5000000  # 5 Mbps (increased for experiments) was 3Mbps
...
class H264Decoder(Decoder):
    ...
    def decode(self, encoded_frame: JitterFrame) -> list[Frame]:
        try:
            ...
            # During packet loss, decode errors are expected - reduce log noise
            if "Invalid data found when processing input" in str(e):
                logger.debug("H264Decoder() decode error (expected during packet loss): " + str(e))
            else:
...
class H264Encoder(Encoder):
    ...
    def _encode_frame(
        ...
        if self.codec and (
            ...
            or (self.codec.bit_rate and
                abs(self.target_bitrate - self.codec.bit_rate) / self.codec.bit_rate > 0.1)
        ):
            self.buffer_data = b""
        ...
        if self.codec is None:
            ...
            # IMPORTANT: for constant QP
            self.codec.bit_rate = 0  # or simply omit setting it
            ...
            # Fixed QP for fair comparison - disable CRF and use constant QP
            self.codec.options = {
                "preset": "fast",
                "tune": "zerolatency",
                "profile": "baseline",
                # Pass all to x264 directly to be safe
                "x264-params": (
                    "qp=23:"
                    "aq-mode=0:"
                    "ipratio=1:"
            ...
            #     "level": "31",
            #     "tune": "zerolatency",
            #     "qp": "23",  # Fixed QP for consistent quality comparison
            #     "preset": "fast",
            #     "b": str(self.target_bitrate),  # Target bitrate
            #     "maxrate": str(int(self.target_bitrate * 1.2)),
            #     "bufsize": str(int(self.target_bitrate * 0.5)),
            # }
